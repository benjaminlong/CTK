...
void checkFinalWidgetState2(void* data)
  ...
  CTKCOMPARE(widget->opacity(), 0.31);
  CTKCOMPARE(widget->ambient(), 0.75);
  CTKCOMPARE(widget->diffuse(), 0.00);
  CTKCOMPARE(widget->specular(), 0.00);
  CTKCOMPARE(widget->backfaceCulling(), true);
  CTKCOMPARE(widget->color(), QColor(71,150,32));
  }
}
...
int ctkMaterialPropertyWidgetEventTranslatorPlayerTest1(int argc, char * argv [] )
{
  ...
  // Test case 1
  ctkMaterialPropertyWidget* widget = new ctkMaterialPropertyWidget() << vvSetName("MaterialProperty1");
  etpWidget.addTestCase(widget,
                        xmlDirectory + "ctkMaterialPropertyWidgetEventTranslatorPlayerTest1.xml",
                        &checkFinalWidgetState);

  // Test case 2
  ctkMaterialPropertyWidget* widget2 = new ctkMaterialPropertyWidget() << vvSetName("MaterialProperty2");
